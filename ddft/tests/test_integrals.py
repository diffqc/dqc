...
def test_integral_shape(int_type):
    kwargs = {
        "dtype": torch.double,
        "device": torch.device("cpu"),
    }
    ...
    # positions
    pos1 = (torch.randn(posshape, **kwargs) * 0.3).requires_grad_()
    pos2 = (torch.randn(posshape, **kwargs) * 0.3).requires_grad_()
    if int_type == cint.nuclattr:
        posc = (torch.randn((3, natoms), **kwargs) + 1).requires_grad_()
    elif int_type == cint.elrep:
        pos3 = (torch.randn(posshape, **kwargs) * 0.3).requires_grad_()
        pos4 = (torch.randn(posshape, **kwargs) * 0.3).requires_grad_()

    # angular momentum: 3
    lmn1 = torch.ones(posshape)
    lmn2 = torch.ones(posshape)
    if int_type == cint.elrep:
        lmn3 = torch.ones(posshape)
        lmn4 = torch.ones(posshape)
